        ...
        if(!$row){ //se la mail non viene trovata mostra il messaggio
            ...
            if(password_verify($passIns,$passTab)){ //controllo che la password inserita nel Form corrisponda a quella nella TABLE
                ...
                if(isset($_SESSION['current_page'])){ /* ho salvato nella V. di Sessione "current_page" la pagina da cui proveniamo per effettuare il login in modo tale che una volta effettuato il login torniamo a tale pagina */
                    $_SESSION['loginFlag'] = "logged"; //la usiamo dentro quiz.php per controllare se siamo loggati poichè se non siamo loggati non si può fare il quiz
                    $x = $_SESSION['current_page']; //la usiamo per poter tornare nella pagina da cui proveniamo 
                    $_SESSION['emailUserLogged'] = $email; /* variabile necessaria per far si che quando facciamo il quiz il risultato del quiz si possa salvare nella tableChronology e la usiamo in Personal Area per indicare i quiz effettuati dall'utente Loggato */ 
                    header("location: $x");
                }else{
